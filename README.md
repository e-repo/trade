...
**Текущие модули:**
- `Trade` - модуль торговой системы с реверсивными аукционами (основной бизнес-модуль)
...
**Авторизация:** Данный шаблон не включает аутентификацию. Предполагается, что авторизация обрабатывается внешним Gateway сервисом. Для идентификации пользователя используется заголовок `x-user-id` с UUID пользователя.
...
- `trade` - таблицы модуля Trade (Lot, Bid, CargoType, Contractor, VolumeStep)
- `module` - таблицы SomeModule (например: Category)
- `corekit` - общие таблицы

Миграции организованы по модулям в `migrations/{Module}/`.

## Модуль Trade - Торговая система

Модуль `Trade` реализует систему реверсивных аукционов для торгов за перевозку грузов.

**📖 Руководство по ручному тестированию:** [`extra/docs/Manual-Testing-Guide.md`](extra/docs/Manual-Testing-Guide.md) - пошаговая инструкция для тестирования полного цикла торгов с готовыми curl-запросами.

### Основные возможности

#### HTTP API Endpoints
- **POST** `/api/v1/trade/lot` - Создание нового лота
- **GET** `/api/v1/trade/lot/{lotId}` - Получение информации о лоте с ID победителей
- **POST** `/api/v1/trade/bid` - Размещение ставки на лот

#### Console Commands
- `trade:open-lots` - Открытие лотов, у которых наступило время начала торгов
- `trade:calculate-winners` - Закрытие лотов с истёкшим сроком и расчёт победителей

### Бизнес-логика

**Лот (Lot):**
- Имеет общий объём груза, стартовую цену и шаг цены
- Проходит через статусы: PENDING → OPEN → CLOSED
- Открывается автоматически по расписанию (opensAt)
- Закрывается автоматически при истечении времени (closesAt)

**Ставка (Bid):**
- Контрагент указывает цену за тонну и желаемый объём
- Автоматически рассчитывается распределённый объём (allocatedVolume)
- Ставки с более низкой ценой имеют приоритет
- При равной цене приоритет имеет ставка, поданная раньше

**Расчёт победителей:**
- Выполняется командой `trade:calculate-winners`
- Использует пессимистические блокировки (SELECT FOR UPDATE) для конкурентной безопасности
- Распределяет объём лота между ставками по алгоритму:
  1. Сортировка по цене (ASC) и времени создания (ASC)
  2. Распределение доступного объёма с учётом шага объёма
  3. Вытеснение более дорогих ставок при появлении дешёвых
- Обработка лотов батчами (по 100 шт) для оптимизации памяти

### Структура модуля Trade

```
Trade/
├── UI/
│   ├── Http/V1/
│   │   ├── Lot/Create/        # Создание лота
│   │   ├── Lot/Get/           # Получение лота
│   │   └── Bid/PlaceBid/      # Размещение ставки
│   └── Console/
│       ├── OpenLotsCommand.php           # Открытие лотов
│       └── CalculateWinnersCommand.php   # Расчёт победителей
├── Application/
...
└── Infra/
    ├── Dictionary/Repository/ # Реализации репозиториев справочников
    └── Lot/Repository/        # LotRepository (с оптимизацией N+1)
```

### Оптимизации производительности

**Батчевая обработка лотов:**
- Метод `findLotsToCloseIterator` использует 3-шаговую загрузку:
  1. SELECT ID лотов с `FOR UPDATE` (блокировка строк)
  2. Загрузка Lot entities через Doctrine (уже заблокированы)
  3. Загрузка всех allocated bids одним IN-запросом
- Устраняет проблему N+1 запросов (было N+1, стало 3 запроса на батч)

**Индексация для O(1) доступа:**
- Лоты индексируются по ID в памяти для быстрого доступа
- Ставки группируются по lotId для эффективной обработки
